...
<head>
	...
	<title><?php _e( 'WordPress &rsaquo; Multisite Installation' ); ?></title>
...
if ( is_blog_installed() ) {
	...
	die(
		...
		'<p class="step"><a href="' . esc_url( admin_url() ) . '" class="button button-large">' . __( 'Log In' ) . '</a></p>' .
		'</body></html>'
	);
}
...
<p><?php _e( 'Please provide the following information. Do not worry, you can always change these settings later.' ); ?></p>
	...
	case 2:
		...
		if ( false === $error ) {
			...
			grant_super_admin( $result["user_id"] );
			?>
...
<p class="step"><a href="<?php echo esc_url( admin_url() ); ?>" class="button button-large"><?php _e( 'Log In' ); ?></a></p>
